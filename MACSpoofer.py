...
Date: 3/12/2021
Written by: Hunter Iddings
Program Description: Change MAC address of desired interface, for Linux use only.
...
# List of vendors MAC addresses for sample MAC output
...
def list_interfaces():
    # Interfaces directory for Linux systems
    interfaces_dir = "/sys/class/net"

    # Get all current interfaces on the system
    interfaces = os.listdir(interfaces_dir)

    interfaces.remove("lo")  # Remove loopback address in output
    ...
    for index, item in enumerate(interfaces):
        # if selected index is a number valid, return the item of that index
        if (select_item == str(index)):
            return item


def get_permanent_mac(interface):  # Get the permanent assigned MAC address
    ethtool_result = subprocess.check_output(["ethtool", "-P", interface])
    # Regular expression to extract MAC address from output
...
def get_current_mac(interface):  # Get the current assigned MAC address
...
# Print out list of potential MAC addresses
...
def change_mac(selected_interface, mac_address):
    # Check if <random> keyword was passed
...
def main():
    ...
    # Make sure script is being ran on Linux systems only
    ...
    # Check if required packages net-tools and ethtool are installed.
    try:
    ...
    if (selected_interface == None):
        ...
        # Get the permanent and current mac of the system
    ...
    # Select a new MAC address from the list, reset, or randomize
